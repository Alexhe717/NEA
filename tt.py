...
EXACT, LOWER, UPPER = 0, 1, 2 #flag constants
...
def assign_memory(max_memory: int = 512) -> None: #assigning maximum memory to TT
    global MAX_TT_SIZE
...
def trim() -> None: #trim down TT when the maximum size is reached
    while len(TT) > MAX_TT_SIZE:
        TT.popitem(last=False) #remove the least recent used hashing

def init_zobrist(dimension: int) -> None:
    if ZOBRIST_TABLE: return 
    
    random.seed(42) #use a fixed random seed for initialisation
...
def key(board_hash: int, side_to_move: int) -> int:
    if side_to_move == 1:
        return board_hash ^ ZOBRIST_TURN #XOR operation
    return board_hash
...
def store(hash_key: int, depth: int, alpha0: float, beta0: float, score: int) -> None:
    ...
    if (current is None) or (depth >= current[DEPTH]):
        ...
        TT.move_to_end(hash_key) #renew this hashing
        if len(TT) > MAX_TT_SIZE:
            trim()
